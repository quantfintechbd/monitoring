...
class AuthOtpController extends Controller
{
    ...
    public function auth(AuthOtpRequest $request)
    {
        try {   
            ...
            // send username & password via mail
            $username = $this->authOtpService->getUserName();
            ...
            Log::info("AuthOtpController@auth - login credentials sent to the user - name: " .($user->name ?? null) ." mobile: " .($user->mobile ?? null) . " ip: ". $request->ip());
            Log::debug("New User Credentials: Username: {$username} and Password: {$password}");
